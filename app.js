...
/* v132: zone work calendar */
(function(){
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(ch){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch];});}
  function plural(n,one,few,many){n=Math.abs(Number(n)||0)%100;var n1=n%10;if(n>10&&n<20)return many;if(n1>1&&n1<5)return few;if(n1===1)return one;return many;}
  function opt(v,l){var e=document.createElement('option');e.value=v;e.textContent=l;return e;}
  function setOpts(sel,ph,rows,current){if(!sel)return;sel.innerHTML='';sel.appendChild(opt('',ph));rows.forEach(function(r){sel.appendChild(opt(String(r.value),r.label));});if(current&&Array.prototype.some.call(sel.options,function(o){return o.value===String(current);})){sel.value=String(current);}}
  function initZoneCalendar(root){var url=root.getAttribute('data-calendar-url')||'data/calendar-data.json', subject=root.querySelector('[data-calendar-subject]'), zone=root.querySelector('[data-calendar-zone]'), month=root.querySelector('[data-calendar-month]'), tag=root.querySelector('[data-calendar-tag]'), start=root.querySelector('[data-calendar-start]'), result=root.querySelector('[data-calendar-result]'), empty=root.querySelector('[data-calendar-empty]'), title=root.querySelector('[data-calendar-title]'), summary=root.querySelector('[data-calendar-summary]'), season=root.querySelector('[data-calendar-season]'), risk=root.querySelector('[data-calendar-risk]'), basis=root.querySelector('[data-calendar-basis]'), care=root.querySelector('[data-calendar-care]'), monthsWrap=root.querySelector('[data-calendar-months]'), reset=root.querySelector('[data-calendar-reset]'), state=null;
    function rowsForSubject(si){if(si==='')return[];var s=state.subjects[Number(si)];return (s&&s.z?s.z:[]).map(function(zi){var z=state.zones[zi]||{};return {value:zi,label:z.n};});}
    function cal(){return zone&&zone.value?state.byZone[String(zone.value)]:null;}
    function refreshLists(){var c=cal(), months=[],tags={};if(c){(c.months||[]).forEach(function(m){months.push({value:m.month,label:m.month});(m.works||[]).forEach(function(w){if(w.tag)tags[w.tag]=true;});});}setOpts(month,'Все месяцы',months,month?month.value:'');setOpts(tag,'Все типы работ',Object.keys(tags).sort(function(a,b){return a.localeCompare(b,'ru');}).map(function(t){return {value:t,label:t};}),tag?tag.value:'');}
    function render(){var c=cal();if(start)start.hidden=true;if(result)result.hidden=true;if(empty)empty.hidden=true;if(!c){if(start)start.hidden=false;return;}var z=state.zones[Number(c.z)]||{}, s=state.subjects[z.s]||{}, mf=month?month.value:'', tf=tag?tag.value:'', cards=[];(c.months||[]).forEach(function(m){if(mf&&m.month!==mf)return;var works=(m.works||[]).filter(function(w){return !tf||w.tag===tf;});if(!works.length)return;cards.push('<article class="calendar-month-card"><div class="calendar-month-head"><h3>'+esc(m.month)+'</h3><span>'+works.length+' '+plural(works.length,'задача','задачи','задач')+'</span></div><ul class="calendar-task-list">'+works.map(function(w){return '<li><span class="calendar-task-tag">'+esc(w.tag)+'</span><p>'+esc(w.text)+'</p></li>';}).join('')+'</ul></article>');});if(!cards.length){if(empty)empty.hidden=false;return;}if(title)title.textContent=s.n+' — '+z.n;if(summary)summary.textContent=z.sum||'Календарь учитывает сезон, риски зоны и практику посадок для участка.';if(season)season.textContent=c.season||'';if(risk)risk.textContent=c.risk||'';if(basis)basis.textContent=c.basis||'';if(care)care.textContent=c.care||'';if(monthsWrap)monthsWrap.innerHTML=cards.join('');if(result)result.hidden=false;}
    fetch(url).then(function(r){if(!r.ok)throw new Error('calendar');return r.json();}).then(function(data){state={subjects:data.subjects||[],zones:data.zones||[],calendars:data.calendars||[],byZone:{}};state.calendars.forEach(function(c){state.byZone[String(c.z)]=c;});setOpts(subject,'Выберите регион',state.subjects.map(function(s,i){return {value:i,label:s.n};}), '');setOpts(zone,'Сначала выберите регион',[], '');setOpts(month,'Все месяцы',[], '');setOpts(tag,'Все типы работ',[], '');var q=new URLSearchParams(window.location.search).get('zone');if(q){var zi=state.zones.findIndex(function(z){return z.id===q;});if(zi>=0){subject.value=String(state.zones[zi].s);setOpts(zone,'Выберите зону',rowsForSubject(subject.value),String(zi));refreshLists();render();}}if(subject)subject.addEventListener('change',function(){setOpts(zone,'Выберите зону',rowsForSubject(subject.value),'');if(zone&&zone.options.length>1)zone.value=zone.options[1].value;refreshLists();render();});if(zone)zone.addEventListener('change',function(){refreshLists();render();});if(month)month.addEventListener('change',render);if(tag)tag.addEventListener('change',render);if(reset)reset.addEventListener('click',function(){subject.value='';setOpts(zone,'Сначала выберите регион',[],'');setOpts(month,'Все месяцы',[],'');setOpts(tag,'Все типы работ',[],'');if(start)start.hidden=false;if(result)result.hidden=true;if(empty)empty.hidden=true;});if(!q&&start)start.hidden=false;}).catch(function(){if(start){start.hidden=false;var h=start.querySelector('h2'),p=start.querySelector('p');if(h)h.textContent='Календарь временно недоступен';if(p)p.textContent='Не удалось загрузить данные календаря.';}});
  }
...
(function(){
  ...
  function plural(n,one,few,many){n=Math.abs(Number(n)||0)%100;var n1=n%10;if(n>10&&n<20)return many;if(n1>1&&n1<5)return few;if(n1===1)return one;return many;}
  function read(){try{var a=JSON.parse(localStorage.getItem(storageKey)||'[]');return Array.isArray(a)?a:[];}catch(e){return [];}}
  ...
  function payloadFromButton(btn){return {
    id:'pl_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,7),
    name:btn.getAttribute('data-planting-name')||'',
    category:btn.getAttribute('data-planting-category')||'',
    recommendation:btn.getAttribute('data-planting-rec')||'',
    place:btn.getAttribute('data-planting-place')||'',
    timing:btn.getAttribute('data-planting-time')||'',
    comment:btn.getAttribute('data-planting-comment')||'',
    sourceTitle:btn.getAttribute('data-planting-source')||cleanTitle(),
    sourceUrl:btn.getAttribute('data-planting-url')||pageUrl(),
  ...
  function updateButtons(){var items=read();var keys={};items.forEach(function(item){keys[keyOf(item)]=true;});Array.prototype.slice.call(document.querySelectorAll('[data-planting-add]')).forEach(function(btn){var added=!!keys[buttonKey(btn)];btn.classList.toggle('is-added',added);btn.textContent=added?'В списке':'В список';btn.setAttribute('aria-pressed',added?'true':'false');});}
  ...
  function initPage(root){var total=root.querySelector('[data-planting-total]'), search=root.querySelector('[data-planting-filter-search]'), rec=root.querySelector('[data-planting-filter-rec]'), cat=root.querySelector('[data-planting-filter-category]'), empty=root.querySelector('[data-planting-empty]'), results=root.querySelector('[data-planting-results]'), summary=root.querySelector('[data-planting-summary]'), groups=root.querySelector('[data-planting-groups]'), printBtn=root.querySelector('[data-planting-print]'), clearBtn=root.querySelector('[data-planting-clear]'), copyBtn=root.querySelector('[data-planting-copy]');
    ...
    function filtered(items){var q=norm(search&&search.value), rv=rec&&rec.value, cv=cat&&cat.value;return items.filter(function(item){if(rv&&item.recommendation!==rv)return false;if(cv&&item.category!==cv)return false;if(q&&norm([item.name,item.category,item.recommendation,item.place,item.timing,item.comment,item.sourceTitle].join(' ')).indexOf(q)===-1)return false;return true;}).sort(function(a,b){return (rank(a.recommendation)-rank(b.recommendation))||a.name.localeCompare(b.name,'ru')||a.sourceTitle.localeCompare(b.sourceTitle,'ru');});}
    function card(item){return '<article class="planting-card"><div class="planting-card-main"><h3>'+esc(item.name)+'</h3><div class="planting-card-badges"><span class="rec-badge" data-rec="'+esc(item.recommendation)+'">'+esc(item.recommendation)+'</span><span class="planner-badge">'+esc(item.category||'Разное')+'</span><span class="planner-badge">'+esc(item.place||'Условия уточняются')+'</span></div><p>'+esc(item.comment||item.timing||'Смотрите исходный справочник для уточнения условий.')+'</p><small>'+esc(item.timing||'Сроки уточняются в справочнике.')+'</small></div><div class="planting-card-side"><a href="'+esc(item.sourceUrl||'#')+'">Открыть источник</a><span>'+esc(item.sourceTitle||'Справочник')+'</span><button type="button" class="planting-remove-btn" data-planting-remove="'+esc(item.id||'')+'">Убрать</button></div></article>';}
    function render(){var items=read();fillCategories(items);var list=filtered(items);if(total)total.textContent=String(items.length);if(!items.length){if(empty)empty.hidden=false;if(results)results.hidden=true;return;}if(empty)empty.hidden=true;if(results)results.hidden=false;if(summary)summary.textContent='Показано '+list.length+' '+plural(list.length,'позиция','позиции','позиций')+' из '+items.length+'.';var by={};list.forEach(function(item){var k=item.recommendation||'Без оценки';(by[k]||(by[k]=[])).push(item);});var order=['Надёжно','Надежно','Рекомендовано','С укрытием / уходом','Рискованно','Без оценки'];var html=[];order.forEach(function(k){var arr=by[k]||[];if(!arr.length)return;html.push('<section class="planting-group"><div class="planting-group-head"><h2>'+esc(k==='Надежно'?'Надёжно':k)+'</h2><span>'+arr.length+' '+plural(arr.length,'позиция','позиции','позиций')+'</span></div><div class="planting-card-list">'+arr.map(card).join('')+'</div></section>');});if(groups)groups.innerHTML=html.join('')||'<section class="planting-empty"><h2>Ничего не найдено</h2><p>Попробуйте изменить поиск или фильтры.</p></section>';}
    if(search)search.addEventListener('input',render);if(rec)rec.addEventListener('change',render);if(cat)cat.addEventListener('change',render);
    ...
    if(copyBtn)copyBtn.addEventListener('click',function(){var text=textExport(filtered(read()));if(!text)return;if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(text).then(function(){copyBtn.textContent='Скопировано';setTimeout(function(){copyBtn.textContent='Скопировать текст';},1600);});}});
  ...
  document.addEventListener('DOMContentLoaded',function(){updateButtons();Array.prototype.slice.call(document.querySelectorAll('[data-planting-list-page]')).forEach(initPage);var observer=new MutationObserver(function(){updateButtons();});observer.observe(document.body,{childList:true,subtree:true});});
